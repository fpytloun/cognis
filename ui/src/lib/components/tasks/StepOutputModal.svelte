<script lang="ts">
  ...
  const content = $derived(stepOutputContent(stepRun));
...
<div class="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/85 px-4 py-6 backdrop-blur" role="presentation">
  ...
  <div bind:this={container} class="relative z-10 w-full max-w-3xl overflow-hidden rounded-[2rem] border border-slate-800 bg-slate-950 shadow-card" role="dialog" aria-modal="true" aria-labelledby="step-output-title">
    ...
    <div class="max-h-[75vh] space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
      ...
      <section>
        <p class="text-xs uppercase tracking-[0.25em] text-slate-500">Final assistant output</p>
        {#if content}
          <div class="prose prose-sm prose-invert mt-3 max-w-none text-slate-300">{@html renderMarkdown(content)}</div>
        {:else}
          <div class="mt-3 rounded-2xl border border-slate-800 bg-slate-900/50 px-4 py-3 text-sm text-slate-400">
            This attempt did not produce a finalized assistant output body.
          </div>
        {/if}
      </section>
    </div>
