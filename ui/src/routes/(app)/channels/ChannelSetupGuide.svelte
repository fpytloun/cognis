...
{#if guide}
  <Card class="p-5">
    <div class="flex items-start justify-between gap-3">
      ...
      {#if docsUrl}
        <a class="inline-flex items-center gap-2 text-sm text-emerald-300 hover:text-emerald-200" href={docsUrl} target="_blank" rel="noreferrer">
    ...
    <div class="mt-4 rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
      ...
      <ol class="mt-3 space-y-2 text-sm text-slate-300">
        {#each guide.steps as step, index}
          <li class="flex gap-3"><span class="text-emerald-300">{index + 1}.</span><span>{step}</span></li>
        {/each}
      </ol>
