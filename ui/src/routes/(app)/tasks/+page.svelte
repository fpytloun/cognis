  ...
  <section class={`min-w-0 space-y-5 ${selectedCount > 0 ? 'pb-[calc(var(--app-shell-bottom-offset,0px)+5.5rem)]' : ''}`}>
      ...
      <div class="flex gap-2 overflow-x-auto pb-2" role="tablist" aria-label="Task columns">
        {#each TASK_BOARD_COLUMNS as column}
          ...
          <button
            ...
            class={`shrink-0 rounded-full border px-4 py-2 text-sm font-medium transition ${mobileActiveColumn === column.id ? 'border-amber-400 bg-amber-500/15 text-white' : 'border-slate-800 bg-slate-900/60 text-slate-300'}`}
          >
            {column.label}
    ...
    <div class="hidden lg:block">
      <div class="overflow-x-auto">
        ...
        {#each TASK_BOARD_COLUMNS as column}
          <section
            class="flex min-h-[600px] flex-col rounded-3xl border p-4 shadow-card transition-colors {dropTargetColumn === column.id && dragState && dragState.column !== column.id ? 'border-amber-500/50 bg-amber-950/20' : 'border-slate-800/80 bg-slate-900/70'}"
