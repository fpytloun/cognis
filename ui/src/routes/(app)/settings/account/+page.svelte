<script lang="ts">
  ...
  // Clean-URL deep link for /settings/account. Redirects to the canonical
  // /settings?tab=account. Three-layer redirect strategy:
  ...
  onMount(() => {
    void goto('/settings?tab=account', { replaceState: true });
  });
</script>

<svelte:head>
  <title>Settings · account · Cognis</title>
  <meta http-equiv="refresh" content="0; url=/settings?tab=account" />
</svelte:head>
