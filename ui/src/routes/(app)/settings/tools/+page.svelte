<script lang="ts">
  ...
  // Clean-URL deep link for /settings/tools. Redirects to the canonical
  // /settings?tab=tools. Three-layer redirect strategy:
  ...
  onMount(() => {
    void goto('/settings?tab=tools', { replaceState: true });
  });
</script>

<svelte:head>
  <title>Settings · tools · Cognis</title>
  <meta http-equiv="refresh" content="0; url=/settings?tab=tools" />
</svelte:head>
