<script lang="ts">
  ...
  // Clean-URL deep link for /settings/routing. Redirects to the canonical
  // /settings?tab=routing. Three-layer redirect strategy:
  ...
  onMount(() => {
    void goto('/settings?tab=routing', { replaceState: true });
  });
</script>

<svelte:head>
  <title>Settings · routing · Cognis</title>
  <meta http-equiv="refresh" content="0; url=/settings?tab=routing" />
</svelte:head>
