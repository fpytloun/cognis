<script lang="ts">
  ...
  // Clean-URL deep link for /settings/web. Redirects to the canonical
  // /settings?tab=web. Three-layer redirect strategy:
  ...
  onMount(() => {
    void goto('/settings?tab=web', { replaceState: true });
  });
</script>

<svelte:head>
  <title>Settings · web · Cognis</title>
  <meta http-equiv="refresh" content="0; url=/settings?tab=web" />
</svelte:head>
