...
def upgrade() -> None:
    ...
    if not inspector.has_table("channel_recipient_intents"):
        op.create_table(
            ...
            sa.UniqueConstraint(
                "user_email", "intent_id", name="uq_channel_recipient_intent_owner"
            ),
        )
    indexes = {
        str(index["name"])
        for index in sa.inspect(op.get_bind()).get_indexes("channel_recipient_intents")
        if index.get("name")
    }
    if "ix_channel_recipient_intents_state" not in indexes:
        op.create_index(
            "ix_channel_recipient_intents_state",
            "channel_recipient_intents",
            ["resolution_state", "updated_at"],
        )
    if "ix_channel_recipient_intents_route" not in indexes:
        op.create_index(
