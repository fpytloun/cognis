...
from cognis.core.tool_output_presentation import build_recovery_hint
...
def build_compacted_tool_result_placeholder(message: dict[str, Any]) -> str:
    ...
    anchor_count_raw = message.get("_anchor_count")
    anchor_count = anchor_count_raw if isinstance(anchor_count_raw, int) else None
...
def should_reproject(
    *,
    ...
    -------------------------
    1. Critical pressure → ``critical_reproject`` always.
...
class ProjectionTurnState:
    ...
    def seed_from_cross_turn_result(
        self, result: ProjectionResult, messages: list[dict[str, Any]]
    ) -> None:
        ...
        compacted_indices = {
            i for i, msg in enumerate(result.messages) if msg.get(PROJECTED_COMPACTED)
        }
        for group in groups:
            if group.assistant_index not in compacted_indices:
                self.committed_preservations.add(_group_anchor(group))
...
def project_messages(
    ...
    *,
    ...
    if is_critical:
        # Critical: only keep the newest same-turn group.
        preserved_assistant_indices = latest_turn_preserved_indices
    else:
        ...
        preserved_assistant_indices = preserved_assistant_indices | committed_indices
