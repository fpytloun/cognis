...
async def _validate_write_deliverable_rich(
    ...
    if is_rich_authoring_action(arguments.get("action")):
        try:
            resolved = await resolve_deliverable_authoring(
                arguments,
                session_factory=context.session_factory,
                artifact_store=context.artifact_store,
                owner_email=context.actor_email or "",
                conversation_id=context.conversation_id or "",
                agent_id=context.conversation_agent_id,
            )
            normalized_rich = resolved.rich
        except DeliverableAuthoringError as exc:
            issues.append(NativeValidationIssue(code=exc.code, path=exc.path, message=exc.message))
        except RichPayloadValidationError as exc:
            issues.extend(_rich_payload_validation_issues(exc, prefix="$.payload"))
    elif arguments.get("format") == "rich" or "rich" in arguments:
        issues.append(
            NativeValidationIssue(
                code="removed_rich_shape",
                path="$",
                message="Use action='rich' with exactly one payload source.",
            )
        )

    format_name = (
        "rich" if normalized_rich is not None else str(arguments.get("format") or "markdown")
    )
    if normalized_rich is None and not is_rich_authoring_action(arguments.get("action")):
        try:
            if format_name == "rich":
...
def _rich_payload_validation_issues(
    exc: Any, *, prefix: str = "$.rich"
) -> list[NativeValidationIssue]:
    issues: list[NativeValidationIssue] = []
    for issue in exc.issues:
        rich_path = str(issue["path"])
        if rich_path == prefix or rich_path.startswith(f"{prefix}."):
            retry_path = rich_path
        else:
            retry_path = f"{prefix}{rich_path[1:]}" if rich_path.startswith("$") else prefix
        issues.append(
            NativeValidationIssue(
                ...
                message=(
                    ...
                    "Correct that path and retry the Rich write_deliverable action."
                ),
            )
        )
    return issues
