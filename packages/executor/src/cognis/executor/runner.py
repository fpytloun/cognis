...
from cognis.executor.tool_resolution import filter_tools_by_executor
from cognis.executor.unary_dedup import UnaryCallConflict, UnaryDedupCache
from cognis.models.config import ImageInput
from cognis.models.executor_calls import EXECUTOR_CALL_SNAPSHOT_MAX_ACTIVE
...
from cognis.providers.llm.terminal import LLMStreamTerminal, response_incomplete_details
...
# Resolve the common MCP runtime only when a configure request supplies servers.
MCPClient = Any


class _UnavailableMCPError(RuntimeError):
    """Placeholder replaced with the MCP SDK error class when loaded."""


MCPClientError: Any = _UnavailableMCPError


def _mcp_module() -> Any:
    from importlib import import_module

    global MCPClientError
    module = import_module("cognis.mcp_runtime")
    if MCPClientError is _UnavailableMCPError:
        MCPClientError = module.MCPClientError
    return module


def _safe_message(value: str, *, limit: int = 500) -> str:
    return str(_mcp_module()._safe_message(value, limit=limit))


def build_mcp_client(*args: Any, **kwargs: Any) -> Any:
    return _mcp_module().build_mcp_client(*args, **kwargs)


def mcp_tools_to_definitions(*args: Any, **kwargs: Any) -> Any:
    return _mcp_module().mcp_tools_to_definitions(*args, **kwargs)


def runtime_mcp_server_key(*args: Any, **kwargs: Any) -> Any:
    return _mcp_module().runtime_mcp_server_key(*args, **kwargs)


def validate_unique_server_names(*args: Any, **kwargs: Any) -> Any:
    return _mcp_module().validate_unique_server_names(*args, **kwargs)
...
# Terminal tool results retained in memory so the controller can fetch an
# outcome after reconnecting to this same executor process.
_MAX_RETAINED_CALL_RESULTS = 256
# Accepted call IDs retained to prove whether a call ever reached this process.
# Only identifiers are stored, so this stays cheap while covering far more
# history than the retained results.
_MAX_ACCEPTED_CALL_IDS = 8192
# Byte ceilings for retained results. A single oversized result is not retained
# at all (it reports terminal_result_unavailable, never "never ran"), and the
# aggregate is bounded so a mobile executor cannot exhaust memory.
_MAX_RETAINED_RESULT_BYTES = 2 * 1024 * 1024
_MAX_RETAINED_RESULTS_TOTAL_BYTES = 32 * 1024 * 1024
# Results are retained only long enough to survive a reconnect. Bounding the
# lifetime also stops sensitive payloads from residing for the process lifetime.
_RETAINED_RESULT_TTL_SECONDS = 600.0
...
def _mcp_tool_contracts(tools: list[ToolDefinition]) -> list[str]:
    return sorted(
        json.dumps(tool.model_dump(mode="json"), sort_keys=True, separators=(",", ":"))
        for tool in tools
    )
...
def _same_turn_tool_call_identity(params: dict[str, Any]) -> tuple[tuple[str, str], str] | None:
    ...
    if (
        ...
        or not turn_id
        or not isinstance(execution_scope_id, str)
...
class _ConnectionSender:
    """Serialize sends for one physical WebSocket connection."""
    ...
    async def send(self, payload: str) -> None:
        if self.retired:
            raise RuntimeError("executor connection sender retired")
...
class ExecutorRunner:
    ...
    def __init__(self, config: ExecutorConfig) -> None:
        ...
        self._call_snapshot_seq = 0
        self._call_records: dict[str, dict[str, Any]] = {}
        self._terminal_call_records: OrderedDict[str, dict[str, Any]] = OrderedDict()
        self._active_calls: dict[str, asyncio.Task[Any]] = {}
        # Tool executions outlive one websocket so an unstable network cannot
        # destroy work in progress. Results are retained briefly so the
        # controller can fetch them after reconnecting to this same process.
        self._durable_tool_tasks: dict[str, asyncio.Task[Any]] = {}
        self._completed_call_results: OrderedDict[str, dict[str, Any]] = OrderedDict()
        self._retained_result_bytes: dict[str, int] = {}
        self._retained_result_expiry: dict[str, float] = {}
        # Every call this process ever accepted. Only this ledger can prove a
        # call never ran, which is what lets the controller safely retry. It is
        # bounded, so overflow is tracked explicitly: once IDs are dropped the
        # executor must stop claiming a call is unknown.
        self._accepted_call_ids: OrderedDict[str, None] = OrderedDict()
        self._accepted_call_ids_overflowed = False
        ...
        self._mcp_client_active_calls: dict[str, int] = {}
        self._mcp_credential_revisions: dict[str, dict[str, str | int]] = {}
        ...
        self._capability_report: RuntimeCapabilityReport | None = None
        ...
        self._connection_senders: dict[int, _ConnectionSender] = {}
        self._retired_sender_ids: set[int] = set()
        self._oauth_loopback_listeners: dict[str, dict[str, Any]] = {}
        self._browser_cleanup_retainer = BrowserManagerCleanupRetainer()

    def stop(self) -> None:
        """Request a graceful shutdown from a signal handler or embedding host."""
        self._running = False
    ...
    async def run(self) -> None:
        ...
        try:
            while self._running:
                # Retention expiry must not depend on having a connection: a
                # long outage would otherwise keep sensitive payloads resident.
            ...
            durable_tool_tasks = tuple(self._durable_tool_tasks.values())
            for task in durable_tool_tasks:
                task.cancel()
            if durable_tool_tasks:
                await asyncio.gather(*durable_tool_tasks, return_exceptions=True)
            self._durable_tool_tasks.clear()
            self._completed_call_results.clear()
            self._retained_result_bytes.clear()
            self._retained_result_expiry.clear()
    ...
    async def _connect_and_serve(self) -> None:
        ...
        # Reconnects are lightweight: an executor process that is already
        # configured keeps its runtime so a flaky link does not force a full
        # reconfigure. The controller decides whether to reconfigure from the
        # config version reported in executor.ready.
        ...
        async with websockets.connect(
            ...
            self._retired_sender_ids.discard(id(ws))
            self._connection_senders[id(ws)] = _ConnectionSender(ws)
            try:
                    ...
                    json.dumps(
                        {
                            ...
                            "params": self._build_ready_params(resource_snapshot),
                            "id": ready_id,
                        }
                    ),
                )
                ...
                if not isinstance(response, dict):
                    raise ValueError("Invalid executor registration response")
                if "error" in response and not isinstance(response["error"], dict):
                    raise ValueError("Invalid executor registration error")
            except BaseException:
                sender = self._connection_senders.pop(id(ws), None)
                if sender is not None:
                    sender.retire()
                self._retired_sender_ids.add(id(ws))
                raise
            ...
            # Existing adapters can receive messages before the controller
            # reinstalls account callbacks. Retain those notifications until
            # the controller restarts each channel after configuration.
            if self._channel_handler is not None:
                self._channel_handler.pause_notifications()
    ...
    async def _message_loop(self, ws: Any) -> None:
        ...
        async for raw_message in ws:
            ...
            if method == "executor.configure":
            ...
            elif method == "executor.mcp.reload_credentials":
                creator = (
                ...
                creator(
                    self._handle_mcp_credential_reload(ws, msg_id, params),
                    method,
            ...
            elif method == "tool.result_fetch":
                if replay_owner:
                    self._create_replay_safe_handler_task(
                        self._handle_tool_result_fetch(ws, msg_id, params),
                        "tool.result_fetch",
                        call_id=replay_call_id,
                        msg_id=msg_id,
                        owner_ws=ws,
                    ...
                    await self._handle_tool_result_fetch(ws, msg_id, params)
                ...
                if original_call_id is not None:
                    ...
                    continue
                call_id = str(params.get("call_id") or msg_id or uuid.uuid4().hex)
                ...
                self._register_call(
                    ...
                    turn_id=(
                        request_runtime_metadata.get("turn_id")
                        if isinstance(request_runtime_metadata, dict)
                        else None
                    ),
                )
                task = self._create_durable_tool_task(
                ...
                def _remove_active_inference(
                    finished: asyncio.Future[Any],
                    key: str = inference_key,
                ) -> None:
                    if self._active_calls.get(key) is finished:
                        self._active_calls.pop(key, None)

                task.add_done_callback(_remove_active_inference)
            elif method == "llm.cancel":
                request_id = str(params.get("request_id") or "")
                cancel_task = self._active_calls.get(f"llm:{request_id}")
                cancelled = False
    ...
    def _create_durable_tool_task(
        ...
        *,
        ...
        """Run a tool execution independently of one websocket lifetime.

        Tool work must survive transport loss: the controller can reconnect and
        fetch the retained result instead of treating a network blip as an
        unknown outcome. Cancellation still comes from ``tool.cancel``, the
        tool's own timeout, or executor shutdown.
        """

        task = asyncio.create_task(coro, name=f"executor-tool-{call_id}")
        self._durable_tool_tasks[call_id] = task

        def _consume_result(done: asyncio.Task[Any]) -> None:
            if self._durable_tool_tasks.get(call_id) is done:
                self._durable_tool_tasks.pop(call_id, None)
            # A task cancelled before its body ran never reaches the handler's
            # own cleanup, so settle here to avoid a permanently "running" call.
            if call_id in self._call_records:
                self._settle_call(call_id, "cancelled" if done.cancelled() else "failed")
            if self._active_calls.get(call_id) is done:
                self._active_calls.pop(call_id, None)
            try:
                done.result()
            except asyncio.CancelledError:
                logger.debug(
                    "Durable tool task cancelled",
                    extra={"extra_data": {"call_id": call_id, "msg_id": msg_id}},
                )
            except ConnectionClosed:
                logger.debug(
                    "Durable tool task could not reply because websocket closed",
                    extra={"extra_data": {"call_id": call_id, "msg_id": msg_id}},
                )
            except Exception:
                logger.exception(
                    "Durable tool task failed",
                    extra={"extra_data": {"call_id": call_id, "msg_id": msg_id}},
                )
    ...
    async def _handle_tool_result_fetch(
        self, ws: Any, msg_id: str | None, params: dict[str, Any]
    ) -> None:
        """Report the current outcome of a previously accepted tool call."""

        call_id = str(params.get("call_id") or "")
        if not call_id:
            await self._send_rpc_error(ws, msg_id, -32602, "tool.result_fetch requires call_id")
            return None
        ...
        self._prune_retained_results()
        retained = self._completed_call_results.get(call_id)
        if retained is not None:
            payload["state"] = "terminal"
            payload["result"] = dict(retained)
        elif call_id in self._call_records or call_id in self._durable_tool_tasks:
            payload["state"] = "active"
        elif call_id in self._accepted_call_ids:
            # The call definitely reached this process but its result is no
            # longer retained. The outcome is unrecoverable, never "never ran".
            payload["state"] = "terminal_result_unavailable"
        elif self._accepted_call_ids_overflowed:
            # History was truncated, so absence proves nothing.
            payload["state"] = "indeterminate"
        else:
            payload["state"] = "unknown"
        await self._send_rpc_result(ws, msg_id, payload)
    ...
    async def _handle_configure(self, ws: Any, msg_id: str | None, params: dict[str, Any]) -> None:
        async with self._runtime_config_lock:
            await self._handle_configure_unlocked(ws, msg_id, params)

    async def _handle_configure_unlocked(
        self, ws: Any, msg_id: str | None, params: dict[str, Any]
    ) -> None:
        ...
        # Re-applying the current version must stay possible: a reconnecting
        # runner keeps its config, and the controller may not have persisted the
        # applied version before the socket dropped. Only an older version is
        # stale, otherwise that mismatch would deadlock configuration.
        ...
        previous_capability_report = self._capability_report
        previous_mcp_credential_revisions = dict(self._mcp_credential_revisions)
        ...
        try:
            ...
            if not component_available("mcp"):
                staged_mcp_clients: dict[str, MCPClient] = {}
                discovered_tools: list[ToolDefinition] = []
                mcp_statuses: list[dict[str, Any]] = []
                mcp_warnings: list[str] = (
                    ["MCP component is unavailable; install cognis-executor[mcp]."]
                    if mcp_servers
                    else []
                )
            else:
            ...
            self._capability_report = previous_capability_report
            self._mcp_credential_revisions = previous_mcp_credential_revisions
        ...
        web_modules = load_component("web")
        if web_modules:
            web_defs = web_modules[0].web_tool_definitions(
                web_backends,
        ...
        try:
            ...
            self._runtime_metadata = {
                ...
                "mcp_credential_revisions": dict(params.get("mcp_credential_revisions") or {}),
            ...
            self._mcp_credential_revisions = dict(params.get("mcp_credential_revisions") or {})
            ...
            if self._inference_handler is None and component_available("inference"):
                from cognis.executor.inference import InferenceHandler

                self._inference_handler = InferenceHandler()
            elif not component_available("inference"):
                self._runtime_metadata["inference_unavailable"] = (
                    "Install cognis-executor[inference] to enable local inference."
                )
            ...
            self._local_inference_enabled = (
                inference_config.local_inference_enabled and component_available("inference")
            )
            if self._channel_handler is None and component_available("channels"):
        ...
        self._capability_report = collect_runtime_capabilities(
            desired_tools=enabled_tools if isinstance(enabled_tools, list) else [],
            observed_tools=self._configured_tool_definitions,
            browser_config=browser_config_dict,
            lsp_enabled=bool(self._runtime_metadata.get("lsp_enabled", True)),
            runtime_metadata=self._runtime_metadata,
        )
        self._runtime_metadata["observed_capabilities"] = self._capability_report.model_dump(
            mode="json"
        )
        ...
        if msg_id is not None:
            await self._send_rpc_result(
                ...
                msg_id,
                {
                    ...
                    "observed_capabilities": self._capability_report.model_dump(mode="json"),
    ...
    async def _handle_mcp_credential_reload(
        self,
        ws: Any,
        msg_id: str | None,
        params: dict[str, Any],
        ...
        try:
            retired_clients: dict[str, MCPClient] | None = None
            async with asyncio.timeout(50.0):
                async with self._runtime_config_lock:
                    retired_clients = await self._handle_mcp_credential_reload_unlocked(
                        ws, msg_id, params
                    )
        except TimeoutError:
            if msg_id is not None:
                await self._send_rpc_result(
                    ws,
                    msg_id,
                    {"state": "busy", "reason": "credential_reload_deadline_exceeded"},
                )
            return None
        if retired_clients:
            await self._close_clients(retired_clients, suppress_cancelled=True)

    async def _handle_mcp_credential_reload_unlocked(
        self,
        ws: Any,
        msg_id: str | None,
        params: dict[str, Any],
    ) -> dict[str, MCPClient] | None:
        """Stage and replace one OAuth MCP client without changing runtime state."""
        ...
        if not self._configured or self._runtime_state not in {"active", "degraded"}:
            await _respond({"state": "executor_unavailable"})
            return None
        server_raw = params.get("server")
        revision_raw = params.get("credential_revision")
        if not isinstance(server_raw, dict) or not isinstance(revision_raw, dict):
            await self._send_rpc_error(
                ws, msg_id, -32602, "MCP server and credential revision are required"
            )
            return None
        try:
            server = MCPServerConfig.model_validate(server_raw)
        except Exception:
            await self._send_rpc_error(ws, msg_id, -32602, "Invalid MCP server configuration")
            return None
        server_id = str(server.server_id or "")
        token_id = revision_raw.get("token_id")
        token_version = revision_raw.get("token_version")
        if (
            not server_id
            ...
            await self._send_rpc_error(ws, msg_id, -32602, "Invalid MCP credential revision")
            return None
        revision: dict[str, str | int] = {
            "token_id": token_id,
            "token_version": token_version,
        }
        current_revision = self._mcp_credential_revisions.get(server_id)
        if current_revision == revision or (
            isinstance(current_revision, dict)
            and current_revision.get("token_id") == token_id
            and int(current_revision.get("token_version") or 0) >= token_version
        ):
            await _respond(
                {
                    "state": "unchanged",
                    "server_id": server_id,
                    "credential_revision": current_revision or revision,
                }
            )
            return None

        secrets_raw = params.get("secrets")
        secrets = (
            {str(key): str(value) for key, value in secrets_raw.items()}
            if isinstance(secrets_raw, dict)
            else {}
        )
        staged_clients, staged_tools, statuses, warnings = await self._prepare_mcp_runtime(
            [server], secrets
        )
        server_key = runtime_mcp_server_key(server)
        staged_client = staged_clients.get(server_key)
        if staged_client is None:
            status = statuses[0] if statuses else {}
            await _respond(
                {
                    "state": "failed",
                    "server_id": server_id,
                    "error_class": status.get("error_class", "connection_failed"),
                    "warnings": warnings[:3],
                }
            )
            return None
        current_client = self._mcp_clients.get(server_key)
        if current_client is None:
            await self._close_clients(staged_clients, suppress_cancelled=True)
            await _respond(
                {
                    "state": "full_reconfigure_required",
                    "server_id": server_id,
                    "reason": "server_not_in_runtime",
                }
            )
            return None
        current_tools = [
            tool for tool in self._configured_tool_definitions if tool.source.server_id == server_id
        ]
        if _mcp_tool_contracts(current_tools) != _mcp_tool_contracts(staged_tools):
            await self._close_clients(staged_clients, suppress_cancelled=True)
            await _respond(
                {
                    "state": "full_reconfigure_required",
        ...
        deadline = asyncio.get_running_loop().time() + max(
            1.0, min(float(server.timeout_seconds), 10.0)
        )
        while (
            self._mcp_client_active_calls.get(server_key, 0) > 0
            and asyncio.get_running_loop().time() < deadline
        ):
            await asyncio.sleep(0.05)
        if self._mcp_client_active_calls.get(server_key, 0) > 0:
            await self._close_clients(staged_clients, suppress_cancelled=True)
            await _respond(
                {
                    "state": "busy",
                    "server_id": server_id,
                    "reason": "in_flight_calls_did_not_drain",
                }
            )
            return None

        self._mcp_clients[server_key] = staged_client
        self._mcp_credential_revisions[server_id] = revision
        self._runtime_metadata["mcp_credential_revisions"] = dict(self._mcp_credential_revisions)
        logger.info(
            "MCP: reloaded credentials without executor reconfigure",
            extra={
                "extra_data": {
                    "server_id": server_id,
                    "token_version": token_version,
        ...
        return {server_key: current_client}
    ...
    async def _handle_tool_execute(
        ...
        if not self._configured or self._runtime_state not in {"active", "degraded"}:
            try:
                ...
                self._active_calls.pop(call_id, None)
                self._settle_call(str(call_id), "failed")
            return
        ...
        if str(call_id) not in self._call_records:
            self._register_call(
                str(call_id),
                str(tool_name),
                turn_id=request_runtime_metadata.get("turn_id"),
            )
        ...
        try:
            ...
            if handler is None:
                ...
                async def _send_tool_chunk(delta: str, stream: str | None) -> None:
                    # Progress is advisory. Losing the socket mid-execution must
                    # not abort the tool; the terminal result is authoritative.
                    with contextlib.suppress(Exception):
                ...
                terminal_state = "failed" if result.is_error else "completed"
        except TimeoutError:
            ...
            result = ToolResult(
                output=(
                    f"MCP authentication failed: {error_detail}"
                    if authorization_required
                    else f"MCP request failed: {error_detail}"
                ),
                is_error=True,
                duration_ms=int((perf_counter() - start) * 1000),
                metadata={
                    "mcp_auth_error": authorization_required,
                    "mcp_error": True,
            ...
            self._active_calls.pop(call_id, None)
            self._settle_call(str(call_id), terminal_state, result=result_payload)

        if result_payload is None:
            return
        # A closed socket must not discard the outcome: it stays fetchable via
        # tool.result_fetch until the retention window expires.
        with contextlib.suppress(Exception):
            async with send_lock:
                await self._send_rpc_result(ws, msg_id, result_payload)
        ...
        async for chunk in self._inference_handler.stream_complete(
            ...
            if chunk.get("done"):
                ...
                incomplete_details = response_incomplete_details(
                    safe_chunk.get("response_incomplete_details")
                )
                if incomplete_details is not None:
                    terminal["response_incomplete_details"] = incomplete_details
                await self._send_ws(
                    ws,
                    json.dumps(
                        {
                            "jsonrpc": "2.0",
                            "method": "llm.done",
                            "params": {"request_id": request_id, **terminal},
                        }
                    ),
                )
            else:
    ...
    async def _handle_channel_start(
        ...
        if self._channel_handler is None:
            ...
            return
        try:
            await self._channel_handler.activate_notification_sender(
                str(params.get("account_id", "")), lambda payload: self._send_ws(ws, payload)
            )
    ...
    async def _heartbeat_loop(self, ws: Any) -> None:
        while self._running:
            ...
            params: dict[str, Any] = {
                ...
                "call_snapshot": self._build_call_snapshot(),
            }
    ...
    def _register_call(self, call_id: str, tool_name: str, *, turn_id: Any = None) -> None:
        ...
        self._terminal_call_records.pop(call_id, None)
        self._call_records[call_id] = record
        self._accepted_call_ids.pop(call_id, None)
        self._accepted_call_ids[call_id] = None
        while len(self._accepted_call_ids) > _MAX_ACCEPTED_CALL_IDS:
            self._accepted_call_ids.popitem(last=False)
            self._accepted_call_ids_overflowed = True
    ...
    def _settle_call(
        ...
        *,
        ...
        record = self._call_records.pop(call_id, None)
        if record is None:
            return
        now = datetime.now(UTC).isoformat()
        ...
        self._terminal_call_records[call_id] = record
        if result is not None:
            self._retain_call_result(call_id, result)
        self._prune_terminal_call_records(datetime.now(UTC))

    def _retain_call_result(self, call_id: str, result: dict[str, Any]) -> None:
        """Keep a terminal tool result fetchable across a reconnect.

        Retention is bounded by count and by bytes. Dropping a result is always
        safe: the accepted-call ledger still proves the call ran, so it can never
        be misreported as never executed.
        """

        self._drop_retained_result(call_id)
        self._prune_retained_results()
        size = _approximate_result_bytes(result)
        if size > _MAX_RETAINED_RESULT_BYTES:
            return
        self._completed_call_results[call_id] = dict(result)
        self._retained_result_bytes[call_id] = size
        self._retained_result_expiry[call_id] = monotonic() + _RETAINED_RESULT_TTL_SECONDS
        while self._completed_call_results and (
            len(self._completed_call_results) > _MAX_RETAINED_CALL_RESULTS
            or sum(self._retained_result_bytes.values()) > _MAX_RETAINED_RESULTS_TOTAL_BYTES
        ):
            evicted_id = next(iter(self._completed_call_results))
            self._drop_retained_result(evicted_id)

    def _drop_retained_result(self, call_id: str) -> None:
        """Remove one retained result and its accounting together."""

        self._completed_call_results.pop(call_id, None)
        self._retained_result_bytes.pop(call_id, None)
        self._retained_result_expiry.pop(call_id, None)

    def _prune_retained_results(self) -> None:
        """Expire retained results that outlived the reconnect window."""

        now = monotonic()
        for call_id, expires_at in tuple(self._retained_result_expiry.items()):
            if expires_at <= now:
                self._drop_retained_result(call_id)

    def _prune_terminal_call_records(self, now: datetime) -> None:
        cutoff = now.timestamp() - 600
        for call_id, record in tuple(self._terminal_call_records.items()):
            terminal_at = datetime.fromisoformat(record["terminal_at"]).timestamp()
            if terminal_at >= cutoff:
                break
            self._terminal_call_records.pop(call_id, None)
        while len(self._terminal_call_records) > 256:
            self._terminal_call_records.popitem(last=False)

    def _build_call_snapshot(self) -> dict[str, Any]:
        self._prune_terminal_call_records(datetime.now(UTC))
        # Snapshots are built on every heartbeat, which gives retained results a
        # real bounded lifetime even while the executor is otherwise idle.
        self._prune_retained_results()
        self._call_snapshot_seq += 1
        active_records = list(self._call_records.values())
        complete = len(active_records) <= EXECUTOR_CALL_SNAPSHOT_MAX_ACTIVE
        return {
            "schema_version": 1,
            "executor_instance_id": self.executor_instance_id,
            "snapshot_seq": self._call_snapshot_seq,
            "complete": complete,
            "active": [
                dict(record) for record in active_records[:EXECUTOR_CALL_SNAPSHOT_MAX_ACTIVE]
            ],
            "recent_terminal": [dict(record) for record in self._terminal_call_records.values()],
        }

    def _build_ready_params(self, resource_snapshot: dict[str, Any] | None) -> dict[str, Any]:
        capability_report = self._capability_report or collect_runtime_capabilities(
            observed_tools=self._configured_tool_definitions,
            lsp_enabled=True,
            runtime_metadata=self._runtime_metadata,
        )
        return {
            ...
            "call_snapshot": self._build_call_snapshot(),
            # Fast-resume evidence: when this process is still configured the
            # controller can skip the executor.configure round trip.
            "configured": self._configured,
            "config_version": self._config_version,
            "runtime_state": self._runtime_state,
            "mcp_credential_revisions": dict(self._mcp_credential_revisions),
            "observed_capabilities": capability_report.model_dump(mode="json"),
        }
    ...
    def _build_mcp_handler(self, tool: ToolDefinition) -> Any:
        async def _handler(arguments: dict[str, Any], _: Any) -> Any:
            server_key = runtime_mcp_server_key(tool.source)
            client = self._mcp_clients[server_key]
            raw_tool_name = tool.source.raw_tool_name or tool.name
            self._mcp_client_active_calls[server_key] = (
                self._mcp_client_active_calls.get(server_key, 0) + 1
            )
            try:
                return await client.call_tool(raw_tool_name, arguments)
            finally:
                remaining = self._mcp_client_active_calls.get(server_key, 1) - 1
                if remaining > 0:
                    self._mcp_client_active_calls[server_key] = remaining
                else:
                    self._mcp_client_active_calls.pop(server_key, None)
    ...
    async def _send_ws(self, ws: Any, payload: str) -> None:
        sender = self._connection_senders.get(id(ws))
        if sender is None:
            if id(ws) in self._retired_sender_ids:
                raise RuntimeError("executor connection sender retired")
            sender = _ConnectionSender(ws)
            self._connection_senders[id(ws)] = sender
        try:
...
def _approximate_result_bytes(result: dict[str, Any]) -> int:
    """Return the encoded size of a complete tool result payload in bytes.

    The whole payload is measured, including nested attachment structures, so
    retention ceilings reflect real memory rather than character counts.
    """

    try:
        # No coercion fallback: a payload that cannot be serialized exactly is
        # treated as oversized and dropped rather than silently undercounted.
        return len(json.dumps(result).encode("utf-8"))
    except Exception:
        return _MAX_RETAINED_RESULT_BYTES + 1
