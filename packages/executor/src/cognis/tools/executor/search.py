...
async def _glob_with_python(base: Path, pattern: str) -> ToolResult:
    return await asyncio.to_thread(_glob_with_python_sync, base, pattern)


def _glob_with_python_sync(base: Path, pattern: str) -> ToolResult:
    try:
...
async def _grep_with_python(
    ...
    return await asyncio.to_thread(_grep_with_python_sync, base, pattern, include_patterns, options)


def _grep_with_python_sync(
    base: Path, pattern: str, include_patterns: list[str], options: _GrepOptions
) -> ToolResult:
