"""Direct web backend — httpx fetch + DDGS metasearch.

This is the zero-config default backend. No API key required.

NOTE: Search uses the ``ddgs`` metasearch library, which scrapes several
search providers. It has no official provider API contract and can break
when a provider changes its frontend.
"""
...
class DirectBackend:
    """Direct HTTP fetch + DDGS metasearch (no API key needed)."""
    ...
    async def search(
        ...
        *,
        ...
        try:
            ...
            return ToolResult(
                output=f"DDGS search failed ({category.replace('_', ' ')}).",
                is_error=True,
                metadata={
                    "backend": "direct",
                    "provider": "ddgs",
...
def _ddgs_failure_category(exc: Exception) -> str:
    """Classify DDGS failures without depending on private exception types."""
    text = f"{type(exc).__name__} {exc}".lower()
    if "no results found" in text:
        return "no_results"
    if isinstance(exc, (TimeoutError, asyncio.TimeoutError, httpx.TimeoutException)) or any(
        token in text for token in ("timeoutexception", "timed out", "timeout")
    ):
        return "timeout"
