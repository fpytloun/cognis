...
class SearxngBackend:
    ...
    async def search(
        ...
        *,
        ...
        query = _query_with_domain_filters(query, opts)
...
def _query_with_domain_filters(query: str, options: dict[str, Any]) -> str:
    domains = sorted(_domain_set(options.get("include_domains")))
    if not domains:
        return query
    site_filter = " OR ".join(f"site:{domain}" for domain in domains)
    return f"({site_filter}) {query}" if len(domains) > 1 else f"site:{domains[0]} {query}"
