...
def _browser_config(runtime_metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "engine": runtime_metadata.get("browser_engine", BROWSER_DEFAULT_ENGINE),
        "runtime": runtime_metadata.get("browser_runtime", BROWSER_DEFAULT_RUNTIME),
        "channel": runtime_metadata.get("browser_channel", BROWSER_DEFAULT_CHANNEL),
...
def build_manager_from_config(runtime_metadata: dict[str, Any]) -> BrowserManager:
    ...
    return BrowserManager(
        ...
        engine=str(cfg.get("engine", BROWSER_DEFAULT_ENGINE)),
        runtime=str(cfg.get("runtime", BROWSER_DEFAULT_RUNTIME)),
        channel=(
            None
            if (channel_value := cfg.get("channel", BROWSER_DEFAULT_CHANNEL)) is None
            else str(channel_value)
        ),
