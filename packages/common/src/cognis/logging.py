...
_COGNIS_HANDLER_MARKER = "_cognis_configured_handler"
...
def setup_logging(level: str = "info", fmt: str = "json") -> None:
    ...
    # Replace only handlers installed by this function. Test capture, platform
    # logging, and embedding applications can attach their own root handlers.
    for handler in root.handlers[:]:
        if getattr(handler, _COGNIS_HANDLER_MARKER, False):
            root.removeHandler(handler)

    handler = logging.StreamHandler(sys.stderr)
    setattr(handler, _COGNIS_HANDLER_MARKER, True)
    if fmt == "json":
