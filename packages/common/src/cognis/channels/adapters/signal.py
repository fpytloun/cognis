...
class SignalAdapter(BaseChannelAdapter):
    ...
    async def send_message(self, message: OutboundMessage) -> str | None:
        ...
        chunks = format_for_signal(message.content, self.capabilities.max_message_length or 10000)
        if not chunks:
            chunks = []
        ...
        for index, chunk in enumerate(chunks or [None]):
            ...
            if not isinstance(chunk_message_id, str) or not chunk_message_id.strip():
                # Signal's send result can contain a failure after another
                # recipient accepted the message. Do not hide that outcome by
                # continuing to later chunks or returning the last ID.
                return None
    ...
    async def _send_direct(self, message: OutboundMessage) -> str | None:
        ...
        try:
            ...
            if isinstance(result, dict) and isinstance(result.get("results"), list):
                ...
                if any(
                    not isinstance(item, dict) or str(item.get("type", "")).upper() != "SUCCESS"
                    for item in result_items
                ):
                    raise SignalDeliveryFailure(sanitize_signal_failure(error_data=result))
            timestamp = result.get("timestamp") if isinstance(result, dict) else None
            return str(timestamp) if isinstance(timestamp, str | int) else ""
        except SignalCliRuntimeError as exc:
            logger.warning(
                "signal adapter: direct send failed",
                extra={"extra_data": {"account_id": self.account_id}},
                exc_info=True,
            )
            if exc.rpc_code is not None or exc.error_data is not None:
