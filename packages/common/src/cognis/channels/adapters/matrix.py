...
class MatrixAdapter(BaseChannelAdapter):
        ...
        *,
        ...
        if msgtype in {"m.image", "m.video", "m.audio", "m.file"}:
            ...
            if encrypted_file:
                ...
                raw_media_info = content.get("info")
                media_info: dict[str, Any] = (
                    raw_media_info if isinstance(raw_media_info, dict) else {}
                )
    ...
    async def _chat_type_for_room(self, room_id: str, room_data: dict[str, Any] | None) -> str:
        ...
        invited_count = summary.get("m.invited_member_count", 0)
        total_members: int | None
        if isinstance(joined_count, (int, str)) and isinstance(invited_count, (int, str)):
            try:
