...
def _render_block(
    ...
    *,
    ...
    if block_type == "hero":
        ...
        parts.append(subtitle)
        if status:
            parts.append(f"Status: {status}" + (f" ({tone})" if tone else ""))
        ...
        progress = _progress_text(block.get("progress"))
        if progress:
...
def _render_table(block: dict[str, Any], *, context: ProjectionContext) -> str:
    ...
    for row in rows:
        if isinstance(row, dict):
            ...
            normalized_rows.append([_table_cell_text(row)])
    if not labels:
...
def _progress_text(value: object) -> str:
    if not isinstance(value, dict):
        return ""
    current = _scalar(value.get("value"))
    maximum = _scalar(value.get("max"))
    if not current or not maximum:
        return ""
    label = _text(value.get("label")) or "Progress"
    return f"{label}: {current} of {maximum}"


def _table_cell_text(value: object) -> str:
    if not isinstance(value, dict) or "type" not in value:
        return _scalar(value)
    cell_type = _text(value.get("type"))
    canonical = _scalar(value.get("value"))
    label = _text(value.get("label"))
    tone = _text(value.get("tone"))
    if cell_type == "progress":
        return _progress_text(value)
    if cell_type == "code":
        return f"`{canonical.replace('`', '\\`')}`"
    rendered = label or canonical
    if cell_type == "badge" and tone:
        return f"{rendered} ({tone})"
    return rendered
