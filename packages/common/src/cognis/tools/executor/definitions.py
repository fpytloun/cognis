...
def _component_definitions(name: str) -> list[ToolDefinition]:
    """Return definitions that do not require an optional runtime component."""
    if name == "lsp":
        return [LSP_TOOL]
    return []
...
def executor_tool_handlers() -> dict[str, Any]:
    """Load executor handlers only when the executor distribution is installed."""
    try:
        from cognis.executor.tool_definitions_runtime import (  # type: ignore[import-untyped]
        ...
        if exc.name != "cognis.executor.tool_definitions_runtime":
            raise
        return {}
    return cast(dict[str, Any], executor_tool_handlers())
