...
def describe_available_tool(
    ...
    if operation is not None:
        ...
        selected = next(
            (
                candidate
                for candidate in tool.descriptor.operations
        ...
        descriptor = {
            "schema_version": tool.descriptor.schema_version,
            "schema_hash": tool.descriptor.schema_hash,
            "authority": tool.descriptor.authority,
            "operation": selected.model_dump(mode="json"),
        }
        if tool.descriptor.output_schema is not None:
            descriptor["output_schema"] = tool.descriptor.output_schema
        if tool.descriptor.annotations:
            descriptor["annotations"] = tool.descriptor.annotations
        return {
            "valid": True,
            **identity,
...
def tool_operation_index(
    tool: ToolDefinition,
    *,
    limit: int = 25,
) -> dict[str, Any]:
    """Return compact operation discovery metadata without operation schemas."""

    assert tool.descriptor is not None
    operations = tool.descriptor.operations
    result: dict[str, Any] = {"operation_count": len(operations)}
    if len(operations) <= 1:
        return result
    capped = operations[:limit]
    result["operations"] = [
        {
        ...
        for operation in capped
    ]
    if len(capped) < len(operations):
        result["operations_truncated"] = True
    return result
...
def _selected_operation(tool: ToolDefinition, arguments: dict[str, Any]) -> Any | None:
    ...
    if selected is None:
        for operation in tool.descriptor.operations:
            if "action" not in (operation.input_schema.get("properties") or {}):
                return operation
