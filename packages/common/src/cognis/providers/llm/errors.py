...
def classify_llm_exception(exc: BaseException) -> MidStreamErrorPayload:
    ...
    if callable(to_payload):
        ...
        if isinstance(raw_payload, dict):
            payload = cast(MidStreamErrorPayload, raw_payload)
    ...
    if isinstance(body, dict):
        ...
        err: dict[str, Any] = raw_error if isinstance(raw_error, dict) else body
    ...
    if param is not None:
        classified_payload["param"] = str(param)
    urls, ids = _artifact_refs_from_exception(exc)
    if urls:
        classified_payload["artifact_urls"] = urls
    if ids:
        classified_payload["artifact_ids"] = ids
...
def reasoning_summary_rejected(payload: MidStreamErrorPayload | dict[str, Any] | None) -> bool:
