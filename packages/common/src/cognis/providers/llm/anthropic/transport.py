...
def _compat_usage(usage: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(input_tokens, int):
        prompt_tokens = input_tokens + sum(
            value
            for value in (
    ...
    if isinstance(input_tokens, int) and isinstance(output_tokens, int):
        result["total_tokens"] = result["prompt_tokens"] + output_tokens
    return result
    ...
    async def _request(
        ...
        *,
        ...
        if self._http_client is None:
            raise AnthropicTransportError(
                "Anthropic Messages transport requires an HTTP client adapter"
            )
        owned_client = self._http_client if self._close_http_client else None
