...
class ToolDefinition(BaseModel):
    ...
    provider_exposure_schema: dict[str, Any] | None = None
    provider_exposure_waiver: str | None = None
    source: ToolSource
...
def derive_native_input_schema(operations: list[NativeToolOperation]) -> dict[str, Any]:
    ...
    default_operations = [
        ...
        if "action" not in (schema.get("properties") or {})
    ]
    if len(default_operations) > 1:
        raise ValueError("native tool can declare at most one actionless default operation")
    for name, schema in zip(names, schemas, strict=True):
        action = (schema.get("properties") or {}).get("action")
        if name in default_operations:
            continue
...
def tool_input_schema(tool: ToolDefinition) -> dict[str, Any]:
    """Return the authoritative schema used for descriptors and validation."""

    assert tool.descriptor is not None
    return tool.descriptor.input_schema


def tool_provider_exposure_schema(tool: ToolDefinition) -> dict[str, Any]:
    """Return an explicitly declared provider schema or the authoritative schema."""

    return tool.provider_exposure_schema or tool_input_schema(tool)
