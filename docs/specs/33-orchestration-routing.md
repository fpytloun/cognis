...
|---|---|---:|---:|---:|---:|---:|
| Main / channel chat | Async-first routing options | Yes, when current answer needs result | Yes, only lightweight non-interactive background work | Yes, if caller must inspect result before continuing | Yes, for visible async interactive work | Yes, preferred for heavier workflow-shaped work |
| Direct / topic chat | Inline + joined specialist work | Yes, common | No / not prompt-visible | Yes, for planning/review/implementation needing interaction | Rare / usually not prompt-visible | Yes, if durable/heavy |
